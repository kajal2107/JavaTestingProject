...
public class SeleniumHelpers extends WaitHelpers {
    ...
    public void clearTextBoxUsingKeys(WebElement e) {
        if (Constants.OS.toUpperCase().contains(Platform.WINDOWS.toString())){
            waitTillElementIsClickable(e).sendKeys(Keys.chord(Keys.CONTROL, "a", Keys.DELETE));
        }else {
            waitTillElementIsClickable(e).sendKeys(Keys.COMMAND, "a");
            waitTillElementIsClickable(e).sendKeys(Keys.DELETE);
        }
    }
    ...
    /**
     * To determine whether WebElement is enabled or not
     *
     * @param targetElement WebElement object
     * @return boolean
     */
    ...
    public String getElementAttributeValue(By by, String attributeName) {
        if (isElementAttributePresent(waitTillElementIsVisible(by, 10), attributeName)) {
            return waitTillElementIsVisible(by, 10).getAttribute(attributeName);
        }
        return "Attribute" + attributeName + " not found";
    }
