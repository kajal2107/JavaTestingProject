...
public class CreateProjectConsultantTests extends BaseTest {


    /*Test 1 : Verify that user having consultant can advance project step from 'Early Conversations' to 'Completed'*/
    @Test
    public void verifyConsultantCanTakeProjectToCompleted() throws InterruptedException {
        ...
        projectMemberAddTeamMembersPO.clearAndFillAddTeamMember(consultantUser);

        Reporter.log("Step 3 - Verify that project member is added successfully and Click on project name to navigate details page");
        Assert.assertEquals(projectDetailsTeamMembersListingPO.getMembersName(consultantUser.getRealFirstName() + " " + consultantUser.getRealLastName()), (consultantUser.getRealFirstName() + " " + consultantUser.getRealLastName()), "project member name doesn't match : ");
        Assert.assertEquals(projectDetailsTeamMembersListingPO.getMembersRoleByMemberName(consultantUser.getRealFirstName() + " " + consultantUser.getRealLastName()), consultantUser.getUserRoles().get(0), "project member role doesn't match : ");
        ...
        Reporter.log("Step 7 - Logout admin user and Login again with consultant");
        headerPO.signOut();
        login.fillLoginDetailsAndPerformLogin(consultantUser.getEmail(), consultantUser.getPassword());

        Reporter.log("Step 8 - Find Project created by consultant and go to project detail page from project listing");
        ...
        Reporter.log("Step 9 - Advance Project step from 'Pending Consultant Endorsement' to 'Prep For Financial Endorsement'"); // Todo bug(#1017)
        ...
        Reporter.log("Step 10 - Logout consultant and Login with admin");
        headerPO.signOut();
        login.fillLoginDetailsAndPerformLogin(adminUser.getEmail(), adminUser.getPassword());

        Reporter.log("Step 11 - Find Project admin and go to project detail page from project listing");
        sideHeader.clickOnProjectsMenu();
        headerPO.searchItemGloballyByName(data.getProjectName());
        projectsListingPO.clickOnProjectName(data.getProjectName());

        Reporter.log("Step 12 - Advance Project step from 'Prep For Financial Endorsement' to 'Pending Financial Endorsement'");
        ...
        Reporter.log("Step 18- Advance Project step from 'Finalizing Completion' to 'Completed'");
        ...
        Reporter.log("Step 19 - Verifying project step");
