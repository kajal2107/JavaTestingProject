...
public class CreateProjectFinancialAnalystTests extends BaseTest {


    /*Test 1 : Verify that user having financial analyst can advance project step from 'Early Conversations' to 'Completed'*/
    @Test
    public void verifyFinancialAnalystCanTakeProjectToCompleted() throws InterruptedException {
        ...
        projectMemberAddTeamMembersPO.clearAndFillAddTeamMember(faUser);

        Reporter.log("Step 3 - verify that project member is added successfully and Click on project name to navigate details page");
        Assert.assertEquals(projectDetailsTeamMembersListingPO.getMembersName(faUser.getRealFirstName() + " " + faUser.getRealLastName()), (faUser.getRealFirstName() + " " + faUser.getRealLastName()), "project member name doesn't match : ");
        Assert.assertEquals(projectDetailsTeamMembersListingPO.getMembersRoleByMemberName(faUser.getRealFirstName() + " " + faUser.getRealLastName()), faUser.getUserRoles().get(0), "project member role doesn't match : ");
        projectDetailsTeamMembersListingPO.clickOnProjectDetailLinkButton();

        Reporter.log("Step 3 - Find Project created by admin and go to project detail page from project listing");
        ...
        Reporter.log("Step 9 - Logout admin user and Login again with financialAnalyst");
        headerPO.signOut();
        login.fillLoginDetailsAndPerformLogin(faUser.getEmail(), faUser.getPassword());

        Reporter.log("Step 10 - Find Project created by financialAnalyst and go to project detail page from project listing");
        sideHeader.clickOnProjectsMenu();
        headerPO.searchItemGloballyByName(data.getProjectName());
        projectsListingPO.clickOnProjectName(data.getProjectName());
        ...
        Reporter.log("Step 12 - Logout financialAnalyst and Login with admin");
        headerPO.signOut();
        login.fillLoginDetailsAndPerformLogin(adminUser.getEmail(), adminUser.getPassword());

        Reporter.log("Step 13 - Find Project created by financialAnalyst and go to project detail page from project listing");
        sideHeader.clickOnProjectsMenu();
        headerPO.searchItemGloballyByName(data.getProjectName());
        projectsListingPO.clickOnProjectName(data.getProjectName());
        ...
        Reporter.log("Step 16 - Advance Project step from 'Pending Finance Confirmation' to 'Active'");
        ...
        Reporter.log("Step 17 - Advance Project step from 'Active' to 'Finalize Completion'");
        ...
        Reporter.log("Step 18 - Logout admin user and Login again with financialAnalyst");
        headerPO.signOut();
        login.fillLoginDetailsAndPerformLogin(faUser.getEmail(), faUser.getPassword());

        Reporter.log("Step 19 - Find Project created by financialAnalyst and go to project detail page from project listing");
        sideHeader.clickOnProjectsMenu();
        headerPO.searchItemGloballyByName(data.getProjectName());
        projectsListingPO.clickOnProjectName(data.getProjectName());

        Reporter.log("Step 20 - Advance Project step from 'Finalizing Completion' to 'Completed'");
        ...
        Reporter.log("Step 21 - Verifying project step");
